        ...
        while (a.x >= 0 && a.x < SIZE && a.y >= 0 && a.y < SIZE && MAP[a.x][a.y])
        {
            ...
            else
            {
                if (flag)
                {
                    ...
                    {
                        if (F)
                            printcircle(b.MAP2WINX(), b.MAP2WINY(), (me == 1) ? black : white, MAP); //改变棋子
                        if (!F)
                            mappadd(b.MAP2WINX(), b.MAP2WINY(), (me == 1) ? black : white, MAP); //如果不输出到屏幕，改变地图数组
...
void WIN(int YOURCOLOR, int balckcount, int whitecount) //判断输赢
{
    ...
    {
        if (YOURCOLOR == black)
            mciSendString("play Music/胜利笑声.wav", NULL, 0, NULL);
        else
            mciSendString("play Music/失败.wav", NULL, 0, NULL);
...
POINT2 Easy() //人机对战中等AI
{
    ...
    {
        if ((WEA[i].x < 2 && WEA[i].y < 2) || (WEA[i].x < 2 && SIZE - WEA[i].y - 1 < 2) || (SIZE - 1 - WEA[i].x < 2 && WEA[i].y < 2) || (SIZE - 1 - WEA[i].x < 2 && SIZE - 1 - WEA[i].y < 2))
            continue;
...
unsigned int zobrist_black[BOARD_SIZE][2] = {
    {1288827737, 282448869},
    {450174869, 1246877580},
    {2187254741, 2285582077},
    {3209756399, 3373088701},
    {4087561172, 1416650225},
    {63068892, 2650566517},
    {3446267773, 2624715758},
    {284220267, 1298341087},
    {3844988796, 2642345446},
    {231107512, 1833054584},
    {202260255, 3928555381},
    {1113080702, 4100775676},
    {30928383, 3376494459},
    {3672532980, 2843567806},
    {133562111, 3034607591},
    {1255276367, 2982969333},
    {344167933, 4184993660},
    {1336409332, 1082687484},
    {2772661342, 1956239283},
    {2484941485, 1704900596},
    {848977913, 1951366502},
    {4077908643, 4240998118},
    {1117582013, 3050272501},
    {1518567399, 3162733525},
    {469883390, 2224652133},
    {1319597891, 1695839998},
    {422254458, 4185718199},
    {214671292, 261291881},
    {2602547038, 1502478333},
    {1316000751, 2630929002},
    {2626813421, 2502245053},
    {2889396207, 1148108789},
    {3839385021, 3874095087},
    {2700238583, 1138155004},
    {3673571291, 2462380970},
    {747907070, 2906124228},
    {4170486510, 3303690175},
    {389504929, 1451716780},
    {3707960113, 2430167988},
    {3558993918, 3901215743},
};
// 各位置上出现白棋时对应的散列码
unsigned int zobrist_white[BOARD_SIZE][2] = {
    {3051562733, 64368366},
    {38323581, 2453241279},
    {3364346607, 1296948607},
    {3703204827, 3623485437},
    {1614315502, 241464730},
    {2082766646, 2012019581},
    {2458434406, 966852588},
    {2112649625, 2168093583},
    {4151533533, 3767791280},
    {3143954409, 2442755583},
    {2220587006, 449705967},
    {1958936565, 1279097596},
    {118095230, 216996063},
    {472218747, 4041784188},
    {1085426173, 262143466},
    {3230752719, 317382617},
    {2700104327, 4136203903},
    {418860053, 1880088319},
    {2697543134, 2003006270},
    {791510990, 3887234175},
    {3181477756, 2968361982},
    {2868116474, 2444664535},
    {340817723, 3521855348},
    {3169642996, 4119199725},
    {2618419652, 3760812031},
    {2846637999, 1628372956},
    {25390829, 2676080633},
    {4104738558, 1607360359},
    {565444529, 2563603838},
    {725565357, 1371864251},
    {622031782, 1089629694},
    {1337229293, 578614760},
    {981450622, 1071873982},
    {3829037038, 537386749},
    {4063751393, 3616764919},
    {2514933677, 4059889658},
    {954909852, 1794932348},
    {2276941646, 2961405868},
    {4071112506, 623640571},
    {1033206781, 457178798},
    {904718843, 3223862253},
    {659389915, 4033243119},
    {647581166, 1812719601},
    {3415502079, 1672556404},
    {1895399153, 3589357500},
    {2467230300, 3042983405},
    {1690369017, 1325453244},
    {859758488, 3913219670},
    {2953326191, 325463935},
    {3113516541, 2644463597},
    {3126157279, 2975514558},
    {912572121, 39379253},
    {2677145304, 2417474465},
    {133361104, 4023311831},
    {3406622653, 2531617660},
    {3311468375, 501969916},
    {847769493, 4094817270},
    {3384827876, 2065592252},
    {824147932, 66018199},
    {1594466267, 2741141180},
    {39776890, 1474229584},
    {2299865081, 2000467901},
    {3711809529, 3712442296},
    {1890467676, 3534938109},
};
// 各位置上翻棋时对应的散列码
...
int evaluation(int board[8][8], int color)
{
    ...
    for (i = 0; i < 8; i++)
        for (j = 0; j < 8; j++)
        {
            ...
            else if (Judge(i, j, 1 - color, board))
            {
                opp_mobil++;
                continue;
            }
    ...
    for (i = 0; i < 8; ++i)
        for (j = 0; j < 8; ++j)
        {
            ...
            if (Judge(i, j, color, board))
            {
                ...
                if (depth <= 1)
                    value = -evaluation(boardnow, 1 - color);
                // 否则，对所形成的局面进行递归搜索
                else
                    value = -alpha_beta_with_hashtable(boardnow, -beta, -alpha, depth - 1, 1 - color, 0).value;
    ...
    for (i = 0; i < 8; ++i)
        for (j = 0; j < 8; ++j)
        {
            ...
            if (Judge(i, j, color, board))
            {
                ...
                // 如果到达预定的搜索深度，直接给出局面估值
                if (depth <= 1)
                    value = -evaluation2(boardnow, 1 - color);
                // 否则，对所形成的局面进行递归搜索
                else
                    value = -alpha_beta_with_hashtable2(boardnow, -beta, -alpha, depth - 1, 1 - color, 0).value;
                copymap(boardnow, board);
...
bool putmouse(POINT2 &m) //重定向鼠标坐标
{
    ...
    else if (m.x > WINDOWS_X - WINDOWS_X / 4 + 2 * BBLACK && m.x < WINDOWS_X - WINDOWS_X / 4 + 3.5 * BBLACK + 10 && m.y > 0.5 * BBLACK - 10 && m.y < BBLACK) //读取存档
    {
        ...
        load_game();
        if (!single)
            STARTVS(MYCOLOR, MOUSE, MOUSE);
        else
        {
            if (diff == 1)
                STARTVS(MYCOLOR, MOUSE, Easy);
            else if (diff == 2)
                STARTVS(MYCOLOR, MOUSE, Middle);
            else
                STARTVS(MYCOLOR, MOUSE, Difficult);
        }
    }
    ...
    if (m.x < PX || m.x > PX + SIZE * BBLACK || m.y < PY || m.y > PY + SIZE * BBLACK)
        flag = false; //如果点击在棋盘外
    else
    {
...
int Playchess(int F, POINT2 WINDOWS2, int &balckcount, int &whitecount) //开始
{
    ...
    if (expect[MAP2.x][MAP2.y]) //有位置可行
    {
        ...
        if (balckcount + whitecount >= SIZE * SIZE || !balckcount || !whitecount)
            return 3;        //如果胜负已分
...
void STARTVS(int YOURCOLOR, POINT2 P1(), POINT2 P2()) //开始对战   (mycolor,mouse,easy)
{
    ...
    Printcount(blackcount, whitecount, NOWCOLOR); //当前形势
    Statistics(NOWCOLOR);
    if (MYCOLOR == white && step == 1)
        goto CX2;
    if (NOWCOLOR == white && !single)
        goto CX2;
    while (true)
    {
        cout << "hello" << endl;
...
void TOI(bool FUORKE) //联机模式
{
    ...
    if (FUORKE) //如果为服务端
    {
        ...
        res = connect(sockConn, (SOCKADDR *)&addrSer, sizeof(SOCKADDR));
    ...
    while (_HOME)
    {
        ...
        case WM_LBUTTONDOWN: //当鼠标左键击下时
            ...
            else if (m.x > WINDOWS_X / 2 - 2 * BBLACK && m.x < WINDOWS_X / 2 + BBLACK && m.y > WINDOWS_Y / 3 && m.y < (int)(WINDOWS_Y / 3 + BBLACK / 2) && _DRMS && _SELECTDIFF) //选择先手
            {
                ...
                init();
                if (diff == 1)
                    STARTVS(black, MOUSE, Easy);
                else if (diff == 2)
                    STARTVS(black, MOUSE, Middle);
                else
                    STARTVS(black, MOUSE, Difficult);
                break;
            }
            else if (m.x > WINDOWS_X / 2 - 2 * BBLACK && m.x < WINDOWS_X / 2 + BBLACK && m.y > WINDOWS_Y / 3 + BBLACK + 2 * bblack && m.y < (int)(WINDOWS_Y / 3 + BBLACK * 1.5 + 2 * bblack) && _DRMS && _SELECTDIFF) //选择后手
            {
                ...
                init();
                if (diff == 1)
                    STARTVS(white, MOUSE, Easy);
                else if (diff == 2)
                    STARTVS(white, MOUSE, Middle);
                else
                    STARTVS(white, MOUSE, Difficult);
                break;
            }
